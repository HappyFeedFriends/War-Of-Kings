...
function GetTeamPlayerCount(iTeam)
	...
	for i = 0, DOTA_MAX_PLAYERS - 1 do
		if PlayerResource:IsValidPlayerID(i)  and PlayerResource:GetTeam(i) == iTeam then
			counter = counter + 1
		end
	end
	return counter
...
function CDOTA_PlayerResource:GetAllPlayerCountAlive()
	local counter = 0
	for i = 0, DOTA_MAX_PLAYERS - 1 do
		if PlayerResource:IsValidPlayerID(i) and PlayerResource:GetSelectedHeroEntity(i):IsAlive() then
			counter = counter + 1
		end
	end
	return counter
...
function CDOTA_PlayerResource:GetAllPlayerCount(func)
	local value = 0
	self:PlayerIterate(function(pID)
		if func then
		 	if func(pID) then 
			value = value + 1
			end
