item_octarine_core_1 = class({
	...
	OnSpellStart 	= function(self)
		if self:GetAbilityName() == 'item_octarine_core_godness' then 
			local nPreviewFX = ParticleManager:CreateParticle( "particles/dark_moon/darkmoon_creep_warning.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
...
modifier_octarine_core_custom = class({
	...
	OnCreated 						= function(self)
		if not self:GetAbility() then return
